...
(defun leave-one-out-tests (&optional (datasets *DEFECT-DATASETS*) &key (distance-func 'cosine-similarity) (repeat 20))
...
(defun run-defect-tests (&optional (datasets *DEFECT-DATASETS*) &key (distance-func 'cosine-similarity) (normalize? NIL))
  (let ((sets (copy-list datasets))
	...
	;; Compass with standard stopping rule
	(let (tmp big-tmp)
	  (dotimes (n 20)
	    (dotimes (k (length data))
	      (push (compass-defect-plain (nth k data) data 1.1 1.1 :distance-func distance-func) tmp))
	    (push tmp big-tmp)
	    (setf tmp nil))
	  (push big-tmp vanilla-compass))
	...
	(let (tmp big-tmp)
	  (dotimes (n 20)
	    (dotimes (k (length data))
	      (push (compass-defect-1up (nth k data) data 1.1 1.1 :distance-func distance-func) tmp))
	    (push tmp big-tmp)
	    (setf tmp nil))
	  (push big-tmp compass-1up))
