...
(defun centroid (cluster &key (distance-func 'cosine-similarity))
  (let* ((this (random-element cluster))
    ...
    (dotimes (n (length this))
	  ...
	  (push (/ (+ (nth n this) (nth n that)) 2) center)
	  (if (eql (nth n this) (nth n that))
	      (push (nth n this) center)
	      (push (random-element (list (nth n this) (nth n that))) center))))
    (reverse center)))

(defun bisecting-k-means (k data)
  (let* ((clusters (meat-processor k (k-means k data)))
	 (largest (largest-cluster clusters))
	 (new-clusters (meat-processor 2 (k-means 2 (nth largest clusters))))
	 (clusters (rmnth largest clusters)))
